...
async def test_update_scenario(client: AsyncClient):
    ...
    objectives = [
        ObjectiveIncomingDto(
            scenario_id=new_project_id, name=str(uuid4()), description=str(uuid4())
        ),
        ObjectiveIncomingDto(
            scenario_id=new_project_id, name=str(uuid4()), description=str(uuid4())
        ),
    ...
    response_content = parse_response_to_dtos_test(response, ScenarioOutgoingDto)
    assert (
        response_content[0].name == new_name
        and response_content[0].project_id == new_project_id
        and response_content[0].objectives[0].name == objectives[0].name
    )
...
async def test_update_scenario_is_default_fails(client: AsyncClient):
    payload = [
        ScenarioIncomingDto(
            id=GenerateUuid.as_uuid(3),
            name="",
            project_id=GenerateUuid.as_uuid(3),
            objectives=[],
            opportunities=[],
            is_default=True,
        ).model_dump(mode="json")
