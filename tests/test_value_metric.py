...
async def test_get_value_metric(client: AsyncClient):
    response = await client.get("/value-metrics/20")
    assert response.status_code == 200
...
async def test_update_value_metric(client: AsyncClient):
    new_name=str(uuid4())
    payload=[ValueMetricIncomingDto(id=1, issue_id=1, name=new_name).model_dump()]
...
async def test_delete_value_metric(client: AsyncClient):
    ...
    response=await client.delete("/value-metrics/2")
