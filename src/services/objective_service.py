...
class ObjectiveService:
    async def create(self, session: AsyncSession, dtos: list[ObjectiveIncomingDto], user_dto: UserIncomingDto) -> list[ObjectiveOutgoingDto]:
    ...
    async def update(self, session: AsyncSession, dtos: list[ObjectiveIncomingDto], user_dto: UserIncomingDto) -> list[ObjectiveOutgoingDto]:
        user = await UserRepository(session).get_or_create(UserMapper.to_entity(user_dto))
    ...
    async def get_all(self, session: AsyncSession, odata_query: Optional[str] = None) -> list[ObjectiveOutgoingDto]:
