...
from typing import Optional, Dict, Any, Union, List, Tuple, Iterator, TypeVar, Protocol
from fastapi import HTTPException
from src.constants import Type
from src.dtos.issue_dtos import IssueOutgoingDto
from src.dtos.edge_dtos import EdgeOutgoingDto
from src.dtos.decision_tree_dtos import EdgeUUIDDto, EndPointNodeDto, DecisionTreeDTO2

logger = logging.getLogger(__name__)


class DecisionProtocol(Protocol):
    options: List[Any]  # Replace Any with a more specific type if needed

class UncertaintyProtocol(Protocol):
    outcomes: List[Any]  # Replace Any with a more specific type if needed

class NodeProtocol(Protocol):
    id: uuid.UUID  # or str, depending on your actual id type
    type: str
    decision: Optional[DecisionProtocol]
    uncertainty: Optional[UncertaintyProtocol]

class EdgeProtocol(Protocol):
    tail_node: 'TailNodeProtocol'
    head_node: 'HeadNodeProtocol'

class TailNodeProtocol(Protocol):
    issue_id: uuid.UUID  # or str, depending on your actual id type

class HeadNodeProtocol(Protocol):
    issue_id: uuid.UUID  # or str, depending on your actual id type    

NodeType = TypeVar('NodeType', bound=NodeProtocol)
EdgeType = TypeVar('EdgeType', bound=EdgeProtocol)


class DecisionTreeGraph2():
    """Decision tree class"""
...
class DecisionTreeCreator2(Generic[NodeProtocol, EdgeProtocol]):
    def __init__(self) -> None:
        ...
        self.node_lookup : Dict[str, NodeProtocol] = {}
        self.endpoint_lookup : Dict[str, EndPointNodeDto] = {}

    @classmethod
    async def initialize(cls, scenario_id:uuid.UUID, nodes:list[NodeType], edges:list[EdgeType]) -> DecisionTreeCreator2:
        instance = cls()
    ...
    async def populate_node_lookup(self, nodes: list[NodeProtocol]) -> Dict[str, NodeProtocol]:
        return {str(node.id): node for node in nodes}

    async def create_decision_tree(self) -> DecisionTreeGraph2:
        return await self.convert_to_decision_tree(scenario_id=self.scenario_id)

    async def create_data_struct(self, nodes:list[NodeType], edges:list[EdgeType]) -> Tuple[List[uuid.UUID], List[EdgeUUIDDto]]:
        node_ids = [node.id for node in nodes]
        edge_dtos = [await self.to_arc_dto(nodes, edge) for edge in edges]
        return node_ids, edge_dtos

    async def to_arc_dto(self, nodes: list[NodeType], edge: EdgeProtocol) -> EdgeUUIDDto:
    ...
    async def copy(self) -> DecisionTreeCreator2:
    ...
    async def get_node_from_uuid(self, uuid: uuid.UUID) -> Optional[NodeProtocol]:
    ...
    async def convert_to_decision_tree(self, scenario_id:uuid.UUID) -> DecisionTreeGraph2:
        ...
        decision_tree = DecisionTreeGraph2(root=root_node)
    ...
    async def to_issue_dtos(self, tree: DecisionTreeGraph2) -> Optional[DecisionTreeDTO2]:
    ...
    async def get_tree_node_issue_dto(self, issue: NodeProtocol | EndPointNodeDto, children: list[DecisionTreeDTO2] | None = None) -> DecisionTreeDTO2:
        return DecisionTreeDTO2(
            tree_node=issue,
            children=children
        )

    async def create_issues_dtos_from_tree(self, tree_data: Dict[str, Any]) -> Optional[DecisionTreeDTO2]:
        ...
        children_dtos: list[DecisionTreeDTO2] = []
