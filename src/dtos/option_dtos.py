...
class OptionMapper:
    ...
    def to_outgoing_dto(entity: Option) -> OptionOutgoingDto:
        return OptionOutgoingDto(
            id=entity.id, name=entity.name, decision_id=entity.decision_id, utility=entity.utility,
        )

    @staticmethod
    def to_entity(dto: OptionIncomingDto) -> Option:
        return Option(id=dto.id, name=dto.name, decision_id=dto.decision_id, utility=dto.utility,)
